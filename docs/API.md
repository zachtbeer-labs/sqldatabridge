...
`ExportOptions.DataTablePrefix` controls the prefix used for exported SQLite data tables. It defaults to `zsb_data`, so `dbo.Customers` becomes `zsb_data_dbo__customers`; set it to `null` or an empty string to omit the prefix.

`ExportOptions.Tables` supports exact source table names and `*` wildcards, for example `dbo.Customers`, `Customers`, `dbo.zz*`, and `*.zz*`. `TableSelection` controls whether those patterns are included (`Only`) or excluded (`AllExcept`, the default).
